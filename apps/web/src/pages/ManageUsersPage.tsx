            ...
            {users.map(user => (
              <tr key={user.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <td style={{ padding: "12px" }}>
                  <span style={{ background: user.role === "ROLE_ADMIN" ? "rgba(59,130,246,0.2)" : "rgba(107,114,128,0.2)", color: user.role === "ROLE_ADMIN" ? "#3b82f6" : "#9ca3af", padding: "4px 8px", borderRadius: "4px", fontSize: "0.85rem" }}>
                    {user.role}
                  </span>
                </td>
                <td style={{ padding: "12px", textAlign: "right" }}>
                  <button onClick={() => handleDeleteUser(user.username, user.id)} style={{ background: "rgba(239,68,68,0.2)", color: "#ef4444", padding: "6px 12px" }}>
                    Delete
                  </button>
                </td>
              </tr>
